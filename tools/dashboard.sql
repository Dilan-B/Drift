...
with
done as (select * from public.tasks where deleted_at is null and done),
live as (select * from public.tasks where deleted_at is null),
first_task as (
  select user_id, min(created_at) as t0 from live group by user_id
)
select json_build_object(
  ...
  'headline', (select json_build_object(
      ...
      'first_signup',    (select min(created_at)::date from auth.users)
  )),
  ...
  'funnel', (select json_build_object(
      ...
      'blocked',     (select count(distinct user_id) from public.analytics_events
                        where event_name = 'blocked_apps_selected'),
  ...
  'money', (select json_build_object(
      'subscribers', (select count(*) from public.profiles where sub_active and deleted_at is null),
      'overrides',   (select count(*) from public.pro_overrides where granted
                        and (expires_at is null or expires_at > now())),
      'redeemed',    (select count(*) from public.redeem_code_uses where removed_at is null)
  )),
