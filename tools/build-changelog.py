...
ROWS = [
    ...
    # ── 2026-09-11 · Android port ─────────────────────
    ("2026-09-11", "dcaf805", "Feature", "Android", "Drift runs on Android",
     "Native project generated and committed alongside ios/. All four Android icons are derived from the single iOS app icon by tools/build-android-icons.js so there is one source of truth - the notification icon has to be a white silhouette because Android discards colour and keeps only the alpha channel, and a coloured one renders as a white blob. Four notification channels replace the single bucket expo-notifications would otherwise invent, so muting the daily streak nudge no longer also mutes a lockbox breach warning.",
     "Unreleased", "Native build"),
    ("2026-09-11", "afe45b5", "Feature", "Android", "App blocking on Android, without Family Controls",
     "Android exposes no API for one app to shield another, so Drift does it itself: a foreground service polling UsageStatsManager at 1Hz, an optional AccessibilityService for instant detection, and a full ShieldActivity - an activity rather than a floating overlay, so the blocked app is paused by the OS instead of left playing underneath. Built as a local Expo module so `expo prebuild --clean` cannot wipe it. The JS interface mirrors iOS ScreenTimeModule exactly, so Drift In, blocked hours and the balance reconcile drive it through calls they already made and no screen needed rewriting. Verified on device, including after a reboot with Drift never opened.",
     "Unreleased", "Native build"),
    ("2026-09-11", "ef7d882", "Bug Fix", "Lockbox", "Lockbox handed out screen time without ever checking the phone (Android)",
     "startMonitoring() threw \"unavailable\" on Android and every call site swallowed it, and onStateChange() returned a no-op - so markDisturbed(), the only route to forfeiting a session, could never fire. Every session ran to settle(\"completed\") and awarded the full reward whether or not the phone went in the box: start a 90-minute session, use the phone throughout, collect 45 minutes. Fixed by porting the iOS motion sensing to SensorManager with the constants copied rather than re-tuned. One real difference: both platforms point z out through the screen but disagree on the sign, so Android tests gravity.z > +0.8g for face-up where iOS tests < -0.8.",
     "Unreleased", "Native build"),
    ("2026-09-11", "0e939db", "Bug Fix", "Screen Time", "Screen-time sync had been failing for every user on both platforms",
     "syncScreenTime warned 'record new has no field updated_at' on every call. schema_v7 declared the column inside `create table IF NOT EXISTS public.screen_time`, but the table already existed from schema_v2, so the whole CREATE was skipped and the column never landed; spent_minutes survived only because it got its own explicit ALTER afterwards. The trigger referencing it was created unconditionally and has thrown ever since. Because the upsert conflicts on (user_id, date), the first write each day INSERTed and succeeded while every later write UPDATEd and failed - which is why the friends leaderboard showed stale screen time rather than none. Confirmed with a PostgREST probe (42703 before, 200 after) rather than from the push output.",
     "Unreleased", "SQL migration"),
    ("2026-09-11", "4042b4c", "Bug Fix", "Infra", "expo-constants was pinned four SDKs ahead of the project",
     "package.json carried expo-constants ^57.0.8 against Expo SDK 54, which expects ~18.0.14 - the only expo-* dependency using a caret. iOS tolerated it; Android would not compile at all, because that version calls expo.modules.kotlin.services.ServiceInterface, which does not exist in SDK 54 expo-modules-core. react-native-svg was also behind and aborted the process during Fabric component registration before a single frame rendered.",
     "Unreleased", "Native build"),
    ("2026-09-11", "82407e9", "Bug Fix", "Updates", "The force-update gate would have locked out every Android install",
     "Check 2 read app_config.min_ios_version with no platform guard, while checks 1 and 3 were iOS-gated. Setting that key for an iOS hotfix would have caught every Android install too and held it behind ForceUpdateModal - which has no dismiss - pointed at an App Store URL it cannot install from. The same class of lockout as 2026-07-29, arriving from a config row instead of a stale app.json. All four keys are now per-store, and the Android ones do not exist yet, so Android is inert by default.",
     "Unreleased", "—"),
    ("2026-09-11", "afe45b5", "Bug Fix", "Android", "Blocking would have worked in every dev build and died in production",
     "SYSTEM_ALERT_WINDOW sat in blockedPermissions from when it looked unused. React Native declares it independently in the debug manifest for the dev overlay, so it was present in every debug build and stripped from release - canDrawOverlays() would have returned false forever, the shield would never have launched, and the permission would not even have appeared in Settings for the user to grant. Caught by diffing the merged debug and release manifests, which is now worth doing before any submission.",
     "Unreleased", "Native build"),
    ("2026-09-11", "603c608", "Bug Fix", "Android", "Calendar sync silently refused to turn on",
     "The toggle flipped on and immediately back off, with no dialog and no error. expo-calendar requests READ_CALENDAR and WRITE_CALENDAR together and gates on hasGrantedPermissions(READ, WRITE) - both - but WRITE was in blockedPermissions, so it could never be granted and toggleCalendar() reverted itself. Drift still never writes to a calendar; the permission is held only because the expo-calendar gate demands it. WRITE_CONTACTS stays blocked, because expo-contacts checks isPermissionPresentInManifest first and asks for READ alone when it is absent.",
     "Unreleased", "Native build"),
    ("2026-09-11", "dcaf805", "Bug Fix", "Billing", "Android users would have hit a paywall they could not buy through",
     "RevenueCat issues one public SDK key per store and they are not interchangeable, but every non-iOS caller got \"not_ios\" and failed closed. Now platform-selected via EXPO_PUBLIC_RC_ANDROID_KEY, deliberately with no hardcoded fallback: the 2026-08-28 note in that file records a stale hardcoded key outliving its app and shipping silently in every build, because a wrong key fails exactly like a missing one except a missing one says so.",
     "Unreleased", "—"),
]
