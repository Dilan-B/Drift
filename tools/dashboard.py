...
tools/dashboard.py — internal Drift metrics, as a self-contained HTML page.
    ...
    python3 tools/dashboard.py          # write dashboard.html and open it
...
def q():
    ...
    r = subprocess.run(
        ["supabase", "db", "query", "--linked", "--file", "tools/dashboard.sql"],
...
<p class="sub">Every account, including ours and the test ones. Treat these as upper bounds.</p>
...
<code>tools/dashboard.py</code>. Internal only: run it again for fresh numbers,
and do not edit this file by hand.</p>
</div></body></html>"""


if __name__ == "__main__":
    OUT.write_text(build(q()))
    print(f"wrote {OUT}")
